...
ImageAutoPolygon::ImageAutoPolygon()
    : _image(nullptr), _data(nullptr), _width(0), _height(0), _scaleFactor(0)
{
}

ImageAutoPolygon::ImageAutoPolygon(cocos2d::Image* image)
    : _image(nullptr), _data(nullptr), _width(0), _height(0), _scaleFactor(0)
{
    _image = image;
    CCASSERT(_image != nullptr, "Error Of ImageAuto Polygon Arg Image");
    CCASSERT(_image->getPixelFormat() == backend::PixelFormat::RGBA8888,
...
std::vector<Vec2> ImageAutoPolygon::trace(const Rect& rect, float threshold)
{
    Vec2 first = findFirstNoneTransparentPixel(rect, threshold);
    return marchSquare(rect, first, threshold);
}

Vec2 ImageAutoPolygon::findFirstNoneTransparentPixel(const Rect& rect,
                                                     float threshold)
{
    bool found = false;
    Vec2 i;
...
unsigned char ImageAutoPolygon::getAlphaByIndex(unsigned int i)
{
    return *(_data + i * 4 + 3);
}
unsigned char ImageAutoPolygon::getAlphaByPos(const Vec2& pos)
{
    return *(_data + ((int)pos.y * _width + (int)pos.x) * 4 + 3);
}

unsigned int ImageAutoPolygon::getSquareValue(unsigned int x,
                                              unsigned int y,
                                              const Rect& rect,
                                              float threshold)
{
    /*
     checking the 2x2 pixel grid, assigning these values to each pixel, if not
     transparent
     +---+---+
     | 1 | 2 |
     +---+---+
     ...
     */
    unsigned int sv = 0;
    // NOTE: due to the way we pick points from texture, rect needs to be
    // smaller, otherwise it goes outside 1 pixel
    auto fixedRect = Rect(rect.origin, rect.size - Size(2, 2));

    Vec2 tl = Vec2(x - 1.0f, y - 1.0f);
    sv +=
        (fixedRect.containsPoint(tl) && getAlphaByPos(tl) > threshold) ? 1 : 0;
    Vec2 tr = Vec2(x - 0.0f, y - 1.0f);
    sv +=
        (fixedRect.containsPoint(tr) && getAlphaByPos(tr) > threshold) ? 2 : 0;
    Vec2 bl = Vec2(x - 1.0f, y - 0.0f);
    sv +=
        (fixedRect.containsPoint(bl) && getAlphaByPos(bl) > threshold) ? 4 : 0;
    Vec2 br = Vec2(x - 0.0f, y - 0.0f);
    sv +=
        (fixedRect.containsPoint(br) && getAlphaByPos(br) > threshold) ? 8 : 0;
    CCASSERT(sv != 0 && sv != 15, "square value should not be 0, or 15");
    return sv;
}

std::vector<cocos2d::Vec2> ImageAutoPolygon::marchSquare(const Rect& rect,
                                                         const Vec2& start,
                                                         float threshold)
{
    int stepx = 0;
    ...
    do
    {
        ...
        switch (sv)
        {
            ...
            case 9:
                /*
                 ...
                 |   | 8 |
                 +---+---+
                 this should normally go UP, but if we already been here, we go
                 down
                */
                // find index from xy;
            ...
            case 6:
                /*
                 ...
                 | 4 |   |
                 +---+---+
                 this normally go RIGHT, but if its coming from UP, it should go
                 LEFT
                 */
                i = getIndexFromPos(curx, cury);
        ...
        {
            _points.push_back(
                Vec2((float)(curx - rect.origin.x) / _scaleFactor,
                     (float)(rect.size.height - cury + rect.origin.y) /
                         _scaleFactor));
        }
...
float ImageAutoPolygon::perpendicularDistance(const cocos2d::Vec2& i,
...
std::vector<cocos2d::Vec2> ImageAutoPolygon::rdp(
    const std::vector<cocos2d::Vec2>& v,
    float optimization)
{
    if (v.size() < 3)
        return v;
...
std::vector<Vec2> ImageAutoPolygon::reduce(const std::vector<Vec2>& points,
                                           const Rect& rect,
                                           float epsilon)
{
    ...
    if (size < 3)
    {
        log("AUTOPOLYGON: cannot reduce points for Image that has less than 3 "
            "points in input, e: %f",
            epsilon);
        return std::vector<Vec2>();
    }
    // if there are less than 9 points (but more than 3), then we don't need to
    // reduce it
    else if (size < 9)
    {
        log("AUTOPOLYGON: cannot reduce points for Image e: %f", epsilon);
        return points;
    }
    ...
    if (last.y > result.front().y &&
        last.getDistance(result.front()) < ep * 0.5f)
    {
...
std::vector<Vec2> ImageAutoPolygon::expand(const std::vector<Vec2>& points,
                                           ...
                                           float epsilon)
{
    ...
    if (size < 3)
    {
        log("AUTOPOLYGON: cannot expand points for Image with less than 3 "
            "points, "
            "e: %f",
            epsilon);
        return {};
    ...
    for (const auto& pt : points)
    {
        subj << ClipperLib::IntPoint(
            static_cast<ClipperLib::cInt>(pt.x * PRECISION),
    ...
    clamp.push_back(
        ClipperLib::IntPoint(static_cast<ClipperLib::cInt>(
                                 rect.size.width / _scaleFactor * PRECISION),
                             0));
    clamp.push_back(
        ClipperLib::IntPoint(static_cast<ClipperLib::cInt>(
                                 rect.size.width / _scaleFactor * PRECISION),
                             static_cast<ClipperLib::cInt>(
                                 rect.size.height / _scaleFactor * PRECISION)));
    clamp.push_back(
        ClipperLib::IntPoint(0,
                             static_cast<ClipperLib::cInt>(
                                 rect.size.height / _scaleFactor * PRECISION)));
...
TrianglesCommand::Triangles ImageAutoPolygon::triangulate(
    const std::vector<Vec2>& points)
{
    // if there are less than 3 points, then we can't triangulate
    if (points.size() < 3)
    {
        log("AUTOPOLYGON: cannot triangulate Image with less than 3 points");
        return TrianglesCommand::Triangles();
    }
    std::vector<p2t::Point*> p2points;
    for (const auto& pt : points)
    {
        auto* p = new (std::nothrow) p2t::Point(pt.x, pt.y);
    ...
    // we won't know the size of verts and indices until we process all of the
    // triangles!
    ...
    for (const auto& ite : tris)
    {
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (found)
            {
                // if we found the same vertex, don't add to verts, but use the
                // same vertex with indices
                indices.push_back(j);
                idx++;
            }
            else
            {
                ...
                auto t2f = Tex2F(0, 0);  // don't worry about tex coords now, we
    ...
    unsigned short* indicesBuf =
        new (std::nothrow) unsigned short[indices.size()];
    memcpy(indicesBuf, indices.data(), indices.size() * sizeof(short));

    // Triangles should really use std::vector and not arrays for verts and
    // indices. Then the above memcpy would not be necessary
    TrianglesCommand::Triangles triangles = {vertsBuf,
                                             indicesBuf,
...
void ImageAutoPolygon::calculateUV(const Rect& rect,
                                   V3F_C4B_T2F* verts,
                                   ssize_t count)
{
    /*
    ...
    CCASSERT(
        _width && _height,
        "please specify width and height for this ImageAutoPolygon instance");
    ...
    for (auto i = verts; i != end; ++i)
    {
        ...
        float v =
            (rect.origin.y + rect.size.height - i->vertices.y * _scaleFactor) /
            texHeight;
        i->texCoords.u = u;
        i->texCoords.v = v;
    }
...
Rect ImageAutoPolygon::getRealRect(const Rect& rect)
{
    Rect realRect = rect;
    // check rect to see if its zero
    if (realRect.equals(Rect::ZERO))
    {
        // if the instance doesn't have width and height, then the whole
        // operation is kaput
        CCASSERT(_height && _width,
                 "Please specify a width and height for this instance before "
                 "using its functions");
...
PolygonInfo ImageAutoPolygon::generateTriangles(const Rect& rect,
                                                float epsilon,
                                                float threshold)
{
...
cocos2d::PolygonInfo ImageAutoPolygon::generatePolygon(cocos2d::Image* image,
                                                       const Rect& rect,
                                                       float epsilon,
                                                       float threshold)
{
    ImageAutoPolygon iap(image);
    return iap.generateTriangles(rect, epsilon, threshold);
}

void ImageAutoPolygon::generatePolygon(
    ...
    float threshold)
{
    auto tmpSprite = Sprite::createWithSpriteFrame(sprite->getSpriteFrame());
...
//    pRender->saveToFile("abss.png", true, [&callback](RenderTexture* texture, const std::string& file){
//        auto info = AutoPolygon::generatePolygon(file);
//        callback(info);
//    });

//    pRender->newImage([&](Image* img) {
////        callback(
////            ImageAutoPolygon::generatePolygon(img, rect, epsilon, threshold));
//    });
}

void ImageAutoPolygon::init(cocos2d::Image* image)
{
    _image = image;
    CCASSERT(_image != nullptr, "Error Of ImageAuto Polygon Arg Image");
    CCASSERT(_image->getPixelFormat() == backend::PixelFormat::RGBA8888,
